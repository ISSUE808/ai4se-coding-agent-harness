...
### 10. 计划内未完成（Task 22）[计划内]
- Task 22 文档（README）——Task 20 机制演示、Task 21 分发均已合并（PR #11/#12）
...
| **WebUI 功能占位项**（会话详情「终端」tab 占位提示（"Task 19 接入"）；Settings「清空会话」danger-zone disabled；上下文栏 Token 明细仅总计）[设计]（顶部搜索框已由用户删除） | ① **终端 tab**：纯函数 `reduceTerminalEvent`（tool:executed / feedback:completed / guardrail:triggered / round:changed / session:status → 终端行，500 行上限），useSessionEvents 暴露 `terminal`，SessionDetail term tab 实时渲染（按 kind 着色）——评审修复：**行 id 500 上限后重复**（max+1 单调）、**wire 帧无 timestamp → 接收时戳注入**（纯函数加 `now` 参数）② **清空会话**：`DELETE /api/sessions` 批量清空（非 running 全删，运行中保留并返回 `keptRunning`），Settings 两步确认按钮 + kept-running 提示 ③ **Token 明细**：`TokenUsage {prompt, completion, cached?}` 全链路——DeepSeek 提取 OpenAI 兼容 usage（含 `prompt_cache_hit_tokens`）、main-loop 每轮 `accumulateUsage` 累积到 `session.tokenUsage`（与 memory 层 `tokenCount` 上下文估算并存，语义注释区分）、SessionDetail 显示输入/输出/缓存命中/总计 + 上下文估计 ④ **新增单会话删除**（用户需求）：`DELETE /api/sessions/:id`（404→409 顺序，running 拒绝）+ `SessionStore.remove` + Dashboard 行删除按钮（running 禁用）+ 删除失败 inline 错误。测试 +19（删除端点 ×4、usage ×4、终端 reducer ×6、api ×2、Dashboard ×3、Settings ×2、hook ×1、SessionDetail ×2） | `d9f9cbf` `46837c9` |
...
| **桌面 portable 后端 cwd 漂移导致配置重启丢失**（portable exe 每次自解压到新 `%TEMP%` 目录 → 后端 spawn 的 cwd 漂移 → projectConfigPath/registry 持久化（均基于 cwd）写入临时目录 → 重启后 baseUrl 空、key 还在——key 在 keytar 系统级不受影响，暴露了凭据与配置的存储分层）[实测] | 后端 spawn 固定稳定 cwd：`buildBackendCommand` 加 `options.cwd` + `DesktopLifecycleDeps.backendCwd`，桌面主进程打包态传 `app.getPath('userData')`（`%APPDATA%\codeharness-desktop`）并 mkdir 防御；dev 态门控 `app.isPackaged` 保持仓库根 `.codeharness.json` 语义。验收：21:52 构建实测配置落盘 + 重启保留（TDD 红→绿，desktop 16/16）。**教训：21:52 重建只出了 Setup 安装器，portable exe 仍是 18:28 旧版——验收必须先确认构建产物指纹（grep asar），否则测的是旧代码** | `211048d` `bd6de0e` `6909df5` |
...
| **Token 明细不实时更新**（会话完成后 Token 使用区仍是旧值/估算，需手动刷新才出现明细——WS 只流 status/rounds，tokenUsage 在 loop 结束时才定稿进 REST 快照；SessionDetail 只在挂载时 fetch 一次）[实测] | SessionDetail 终态重取：`snapshotStatusRef` 记录当前快照反映的状态，`events.status` 转 `completed`/`failed` 时重取 REST 快照**恰好一次**（守卫先于 await 防双发；离开终态 re-arm，恢复后再完成可再取；挂载即终态的历史视图不取）；**评审 Important**：重取走 `load('refresh')` 模式不翻转 phase——后台失败静默保留旧视图（全屏错误只在初始加载）；**评审 Minor**：failed 与 completed 同类纳入、fetch 代际守卫（被 supersede 的在途结果丢弃，防 re-arm 竞态）。测试 +5（保活/失败态/挂载终态不取/恢复后再取/原明细测试） | `efad265` `5fcdd0b` |
| **单会话删除无确认**（Dashboard 垃圾桶一次点击即删，误操作不可逆；列表行内高频操作区）[实测] | 两步确认：首击进入该行 armed 态（按钮变「确认删除」dangerSoft 底、宽度自动扩展），3 秒自动解除，再击才调 DELETE；同一时刻仅一行可 armed（点击他行转移）；running 禁用规则不变。测试：两步（首击不发请求）+ 单行 armed 转移；既有删除测试改为两步 | `efad265` `5fcdd0b` |
| **CLI 对话输出杂乱**（REPL 里 readline 回显后 `[user]` 又重复打印一遍；纯工具调用的 assistant 消息打空头；`[session] running/completed` 状态行是噪音——done 摘要已含终态；对话内容与系统消息无视觉区分）[实测] | 降噪 + 着色：① REPL 过滤 `[user]` 回声（**CR Minor 2 边界**：终端回显只发生在 TTY——管道输入 `echoInput=false` 保留 `[user]` 行，脚本捕获不丢指令；start 单跑模式始终保留 `[user]`）② `formatMessageLine` 空 content 且无 toolName → null 不打空头 ③ onStatus 只打非 running/completed 状态（paused/failed 保留——暂停指引前导）④ TTY 下标签着色：user 绿 / assistant 青 / tool·system·feedback·[session] 灰（管道/重定向纯文本，脚本捕获零污染）。测试 +6（formatMessageLine 单元 ×5 含 feedback 灰、REPL 降噪、管道 echoInput、着色 e2e ×2、createTerminalReplIO echoInput 检测）；评审 Ready to merge（5 Minor 修 4：feedback 灰 / echoInput / 着色 e2e / 单跑空头断言；ANSI 常量归属按评审认可保留） | `009ee38` `2d8cd4e` |
| **WebUI 添加供应商后重启 baseUrl 丢失**（行还在、key 还在——key 在 keytar 持久——但 baseUrl 空。根因：POST /api/keys 的 registry 写入走 `persistConfig` 回调，生产链路（`createProgram` → `runWebAction`）从未提供 → `createWebHarness` 内 no-op，只更新内存 liveConfig；而 PUT /api/config 的 config 路由自带默认写盘——两套持久化不对称）[实测] | `runWebAction` 提供默认 `persistConfig`（`createDefaultPersistConfig`，写 `projectConfigPath ?? cwd/.codeharness.json`，格式与 config.ts 缺省逐字节一致、路径与 loader 读取同源）；集成回归测试（waitForShutdown 钩子解析端口 + 真实 fetch POST /api/keys/nju + 文件落盘断言，修复前红）。评审 Approved（2 LOW defer：① 完整配置含用户级 webui.token 会复制进项目文件——与 PUT /api/config 既有缺省行为一致，非本修复引入 ② persist 通道无串行化——既有设计） | `5cb74ce` |
